...
import type { SerpEnrichmentResult, WebContactCandidate, AcceptedContact } from '../../types/owners';

interface SerpEnrichmentPanelProps {
  ownerId: string;
  result: SerpEnrichmentResult;
...
function SourceLabel({ source }: { source: string }) {
  const label = source === 'serpapi_page' ? 'Page' : 'SERP';
  const color = source === 'serpapi_page' ? 'bg-amber-50 text-amber-700' : 'bg-orange-50 text-orange-700';
  return (
...
function SerpCandidateCard({
  candidate,
  ownerId,
  onAccepted,
}: {
  candidate: WebContactCandidate & { source?: string };
  ownerId: string;
  ...
  const handleAccept = async () => {
    ...
    setError(null);
    try {
      const src = (candidate as { source?: string }).source || 'serpapi_serp';
      await acceptContact({
        ownerId,
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <SourceLabel source={(candidate as { source?: string }).source || 'serpapi_serp'} />
            <span className="text-[10px] text-slate-300">
              {Math.round(candidate.confidence * 100)}%
            </span>
          </div>
          <p
            className="text-[10px] text-slate-400 mt-1 leading-relaxed font-mono truncate"
            title={candidate.evidenceSnippet}
          >
...
function SourceGroup({
  ...
  sourceUrl: string;
  title: string;
  candidates: (WebContactCandidate & { source?: string })[];
  ownerId: string;
  onAccepted: (info: AcceptedContact) => void;
}) {
  ...
  return (
    <div className="border border-slate-100 rounded-lg overflow-hidden">
      ...
      >
        <Search className="h-3 w-3 text-orange-400 flex-shrink-0" />
        <span className="text-xs font-medium text-slate-600 truncate flex-1">{title}</span>
        <span className="text-[10px] text-slate-400 flex-shrink-0">
          {candidates.length} contact{candidates.length !== 1 ? 's' : ''}
        </span>
...
export default function SerpEnrichmentPanel({ ownerId, result, onContactAccepted, onRetry, retrying }: SerpEnrichmentPanelProps) {
  const [showSources, setShowSources] = useState(false);

  const grouped = new Map<string, { title: string; candidates: (WebContactCandidate & { source?: string })[] }>();
  for (const c of result.candidates) {
    ...
    if (existing) {
      ...
      grouped.set(c.sourceUrl, { title, candidates: [c] });
    }
  }
  ...
  const addrCount = result.candidates.filter((c) => c.type === 'address').length;
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 mb-2.5">
        ...
        <h4 className="text-xs font-semibold text-slate-500 uppercase tracking-wide">SERP Enrichment Results</h4>
        {result.cached && (
        ...
        {result.cached && onRetry && (
          <button
            onClick={onRetry}
            disabled={retrying}
            className="ml-auto inline-flex items-center gap-1 px-2 py-1 text-[10px] font-medium text-orange-600 hover:text-orange-800 hover:bg-orange-50 rounded transition-colors disabled:opacity-50"
          >
      ...
      {kg && (kg.title || kg.website || kg.phone || kg.address) && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
          <div className="flex items-center gap-2 mb-2">
            <Globe className="h-3.5 w-3.5 text-orange-500" />
            <span className="text-xs font-semibold text-orange-800">Knowledge Graph</span>
          </div>
          <div className="grid gap-1.5 text-xs text-orange-900">
            {kg.title && <div className="font-medium">{kg.title}</div>}
            {kg.description && <div className="text-orange-700 text-[11px]">{kg.description}</div>}
            {kg.website && (
              <div className="flex items-center gap-1.5">
                <Globe className="h-3 w-3 text-orange-400" />
                <a href={kg.website.startsWith('http') ? kg.website : `https://${kg.website}`} target="_blank" rel="noopener noreferrer" className="text-teal-700 hover:underline">
                  {kg.website}
                </a>
              </div>
            )}
      ...
      <div className="flex items-center gap-3 text-xs text-slate-500">
        ...
        {addrCount > 0 && (
          <>
            <span className="text-slate-200">|</span>
            <span>{addrCount} address{addrCount !== 1 ? 'es' : ''}</span>
          </>
        )}
      </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="space-y-1">
              ...
              {result.warnings.length > 5 && (
                <p className="text-[10px] text-amber-500">+{result.warnings.length - 5} more</p>
              )}
            </div>
          </div>
        </div>
      ...
      {result.candidates.length === 0 ? (
        <div className="bg-slate-50 border border-dashed border-slate-200 rounded-lg px-4 py-6 text-center">
          ...
          <p className="text-xs text-slate-400">No contact information found in SERP results.</p>
          {onRetry && (
            <button
              onClick={onRetry}
              disabled={retrying}
              className="mt-3 inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-orange-700 bg-orange-50 border border-orange-200 rounded-lg hover:bg-orange-100 disabled:opacity-50 transition-colors"
            >
      ...
      <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
        <div className="flex items-start gap-2">
          ...
          <p className="text-[10px] text-orange-700">
            SERP-sourced contacts are extracted from Google search results and publicly accessible pages via SerpApi.
            Each result includes evidence from the source. Verify before accepting.
          </p>
        </div>
      </div>
    </div>
