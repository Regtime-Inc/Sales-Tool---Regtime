...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const normalizedNew = normalizeForDedup(value, contactType);
    const alreadyExists = existing.some(
      (c) => normalizeForDedup(c.value, contactType) === normalizedNew
    );

    if (alreadyExists) {
      return new Response(
        JSON.stringify({ ok: true, message: "Contact already exists", duplicate: true }),
    ...
    const newEntry: ContactEntry = {
      value,
      source: source || "web_enrichment",
      confidence: Math.min(confidence || 0.65, 0.75),
    ...
    return new Response(
      JSON.stringify({ ok: true, message: "Contact accepted", entry: newEntry }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    console.error("owner-accept-contact error:", message);
    return new Response(
